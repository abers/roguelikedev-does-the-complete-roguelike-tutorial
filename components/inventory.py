...
class Inventory:
    ...
    def add_item(self, item, colors):
        ...
        if len(self.items) >= self.capacity:
            ...
            results.append({
                'item_added': item,
                'message': Message(f'You pick up the {item.name}!', colors.get('blue'))
            })
    ...
    def use(self, item_entity, colors, **kwargs):
        ...
        if item_component.use_function is None:
            results.append({'message': Message(f'The {item_entity.name} cannot be used', colors.get('yellow'))})
        else:
    ...
    def drop_item(self, item, colors):
        ...
        results.append({'item_dropped': item, 'message': Message(f'You dropped the {item.name}', colors.get('yellow'))})
        return results
